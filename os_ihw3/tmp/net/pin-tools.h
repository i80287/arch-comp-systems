...
static inline bool send_not_croocked_pin(SingleServerWorker worker,
                                         Pin pin) {
...
static inline bool receive_not_crooked_pin(DualServerWorker worker,
                                           Pin* rec_pin) {
    return receive_pin(worker->worker_sock_fd,
                       &worker->first_server_sock_addr, rec_pin);
}
...
static inline bool send_sharpened_pin(DualServerWorker worker, Pin pin) {
    return send_pin(worker->worker_sock_fd,
                    &worker->second_server_sock_addr, pin);
}

static inline bool receive_sharpened_pin(SingleServerWorker worker,
                                         Pin* rec_pin) {
