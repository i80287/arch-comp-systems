...
static void client_handle_errno(const char* cause) {
...
bool send_pin(int sock_fd, const struct sockaddr_in* sock_addr, Pin pin) {
    ...
    buffer.pin   = pin;
    bool success = sendto(sock_fd, buffer.bytes, sizeof(pin), MSG_NOSIGNAL,
...
bool receive_pin(int sock_fd, Pin* rec_pin) {
    ...
    ssize_t read_bytes = recv(sock_fd, buffer.bytes, sizeof(*rec_pin), MSG_NOSIGNAL);
    if (read_bytes != sizeof(*rec_pin)) {
        handle_recv_error(buffer.bytes, read_bytes);
        return false;
    }
...
bool receive_server_log(Client logs_collector, ServerLog* log) {
    char* log_as_bytes = (char*)log;
    ssize_t read_bytes;
    do {
        read_bytes = recv(logs_collector->client_sock_fd, log_as_bytes, sizeof(*log), MSG_NOSIGNAL);
        if (read_bytes != sizeof(*log) && read_bytes != 0) {
            handle_recv_error(log_as_bytes, read_bytes);
            return false;
        }
    } while (read_bytes == 0);
