...
parse_result_t parse_arguments(int argc, const char* const argv[]) {
    ...
    char* end_ptr          = NULL;
    long long res_start_n1 = strtoll(argv[1], &end_ptr, 10);
    result.start_n1        = (uint32_t)res_start_n1;
    if (end_ptr == argv[1] || result.start_n1 != res_start_n1 || errno == ERANGE) {
    ...
    end_ptr              = NULL;
    long long res_end_n2 = strtoll(argv[2], &end_ptr, 10);
    result.end_n2        = (uint32_t)res_end_n2;
    if (end_ptr == argv[2] || result.end_n2 != res_end_n2 || errno == ERANGE) {
        /// If no chars have been read
        //  or
...
void handle_parsing_error(parse_status_t status) {
    if (status & TOO_FEW_ARGS) {
        fputs("[>>>] Error: too few is provided (2 expected)\n", stderr);
    }
    if (status & TOO_MANY_ARGS) {
        fputs("[>>>] Error: too many arguments are provided (2 expected)\n", stderr);
    }
    ...
    fputs(
        "[>>>] Usage hint   : executable start_index end_index\n"
        "[>>>] Usage example: main.out 5 10\n"
