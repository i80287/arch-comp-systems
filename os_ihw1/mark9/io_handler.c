...
static bool run_impl(int read_file_fd, int reader_to_proc_fd, int write_file_fd,
    ...
    if (reader_to_proc_fd < 0) {
        print_failure_message(
            "File reader: can't open fifo [file reader] -> [text processor]");
        return false;
    }
    if (write_file_fd < 0) {
    ...
    if (processor_to_writer_fd < 0) {
        print_failure_message(
            "File writer: can't open fifo [text processor] -> [file writer]");
        return false;
    }
    ...
    while (true) {
        ...
        if (write(reader_to_proc_fd, read_buffer, (size_t)nbytes_read) < 0) {
            print_failure_message(
                "File reader: can't send data to another process using fifo [file "
                "reader] -> [text processor]");
            return false;
        }

        nbytes_read = read(processor_to_writer_fd, read_buffer, TRANSFER_BUFFER_SIZE - 1);
        if (nbytes_read < 0) {
            print_failure_message(
                "File writer: can't read from the fifo [text processor] -> [file "
                "writer]");
            return false;
        }
...
static void notify_end_of_communication(int reader_to_proc_fd) {
    if (reader_to_proc_fd >= 0) {
        const char end_message[] = { END_OF_COMMUNICATION_CHAR };
        write(reader_to_proc_fd, end_message, sizeof(end_message));
    }
}
...
static bool run(const char* input_filename, const char* output_filename) {
    ...
    bool result = run_impl(read_file_fd, reader_to_proc_fd, write_file_fd, processor_to_writer_fd);
    // Send message about end of communication between processes
    notify_end_of_communication(reader_to_proc_fd);
    ...
    if (reader_to_proc_fd >= 0) {
        if (close(reader_to_proc_fd) < 0) {
            print_failure_message(
                "File reader: can't close writing side of the fifo [file reader] "
                "-> "
                "[text processor]");
            result = false;
        }
    ...
    if (processor_to_writer_fd >= 0) {
        if (close(processor_to_writer_fd) < 0) {
            print_failure_message(
                "File writer: can't close opened fifo [text processor] -> [file "
                "writer]");
            result = false;
        }
    }
