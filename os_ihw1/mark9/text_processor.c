...
static bool run_impl(uint32_t start_n, uint32_t end_n, int reader_to_proc_fifo_fd,
    ...
    if (reader_to_proc_fifo_fd < 0) {
        print_failure_message(
            "Data processor: can't open fifo [file reader] -> [text processor]");
        return false;
    }
    if (proc_to_writer_fifo_fd < 0) {
        print_failure_message(
            "Data processor: can't open fifo [text processor] -> [file writer]");
        return false;
    }
    ...
    for (size_t current_index = 0;;) {
        ...
        ssize_t nbytes_read =
            read(reader_to_proc_fifo_fd, read_buffer, TRANSFER_BUFFER_SIZE - 1);
        if (nbytes_read < 0) {
            print_failure_message(
                "Data processor: can't read from fifo [file reader] -> [text "
                "processor]");
            return false;
        }
        if (nbytes_read == 0) {
        ...
        bool end_of_communication = read_buffer[string_length] == END_OF_COMMUNICATION_CHAR;
        ...
        if (write(proc_to_writer_fifo_fd, read_buffer, string_length) < 0) {
            print_failure_message(
                "Data processor: can't send data to another process using fifo [text "
...
static bool run(parse_result_t parse_result) {
    ...
    if (reader_to_proc_fifo_fd >= 0) {
        if (close(reader_to_proc_fifo_fd) < 0) {
            print_failure_message(
                "Data processor: can't open close opened fifo  [file reader] -> "
                "[text "
                "processor]");
            result = false;
        }
    ...
    if (proc_to_writer_fifo_fd >= 0) {
        if (close(proc_to_writer_fifo_fd) < 0) {
            print_failure_message(
                "Data processor: can't open close opened fifo [text processor] -> "
                "[file writer]");
            result = false;
        }
    }
