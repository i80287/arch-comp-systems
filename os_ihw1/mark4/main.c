...
static bool create_file_reader_impl(int* read_file_fd, int fd_reader_to_processor[2]) {
...
static pid_t create_file_reader(const char* input_filename,
    ...
    bool result      = create_file_reader_impl(&read_file_fd, fd_reader_to_processor);
    if (read_file_fd >= 0) {
        close(read_file_fd);
    }
...
static bool create_file_writer_impl(int* write_file_fd, int fd_processor_to_writer[2]) {
    if (close(fd_processor_to_writer[1]) < 0) {
        print_failure_message(
    ...
    char read_buffer[BUFFER_SIZE];
    ssize_t nbytes_read = read(fd_processor_to_writer[0], read_buffer, BUFFER_SIZE - 1);
    if (nbytes_read < 0) {
        print_failure_message(
...
static pid_t create_file_writer(const char* output_filename,
    ...
    bool result       = create_file_writer_impl(&write_file_fd, fd_processor_to_writer);
...
static bool run(parse_result_t parse_result) {
    ...
    pid_t file_reader_pid =
        create_file_reader(parse_result.input_filename, fd_reader_to_processor);
    ...
    pid_t file_writer_pid =
        create_file_writer(parse_result.output_filename, fd_processor_to_writer);
