...
int main(void) {
    ...
    if (shared_mem_id == -1) {
        perror("[client] [>>>] error: shm_open");
        return EXIT_FAILURE;
    }
    if (ftruncate(shared_mem_id, SHARED_MEMORY_SIZE) == -1) {
        perror("[client] [>>>] error: ftruncate");
        shm_unlink(shared_object_name);
        return EXIT_FAILURE;
    }

    printf("[client] [>>>] got access to the shared memory with key %d and size %d bytes\n",
        shared_mem_id,
        SHARED_MEMORY_SIZE);
